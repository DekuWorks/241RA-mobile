...
| Map view | 🔄 | Needs `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY`; rebuild dev client after `react-native-maps` install |
...
2. **Set Google Maps API key** (`EXPO_PUBLIC_GOOGLE_MAPS_API_KEY`) for map screen; run `npx expo prebuild` and rebuild dev client
